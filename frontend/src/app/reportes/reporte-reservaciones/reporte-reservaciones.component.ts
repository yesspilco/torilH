...
export class ReporteReservacionesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.bandera = 0;
    this.h = 1;
    this.empleadosService.listarReservaciones().subscribe(data1 => {
      if (data1.length != 0) {
  ...
  buscarPorFechas() {
    ...
    this.empleadosService.reservaPorFechas(this.FechaInicio, this.FechaFin).subscribe(data => {
      if (data.length != 0) {
